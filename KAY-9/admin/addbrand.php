...
{
    header("location: ../admin/admin-brand.php");
    die();
}
}
...
if(isset($_POST['submit'])){
    ...
    $sql = "SELECT * FROM `brands` WHERE `brand_name`='$name' and `brand_status`='$status'";
    $result = mysqli_query($conn,$sql);
    $num = mysqli_num_rows($result);
    if($num > 0){
        if(isset($_GET['id']) && $_GET['id']!=''){
    ...
    if(isset($_GET['id']) && $_GET['id']!=''){
        ...
        mysqli_query($conn,"UPDATE `brands` SET `brand_name` = '$name', `brand_status` = '$status' WHERE `brands`.`brand_id` = '$id'");
        
    }else
    {
        
        mysqli_query($conn,"INSERT INTO `brands` (`brand_name`, `brand_status`) VALUES ('$name', '$status');");
